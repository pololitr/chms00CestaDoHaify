...
public class PrikazUse implements IPrikaz
{
    ...
    private Batoh batoh;
    private AudioClip airplane;
    private AudioClip manual;
    String vypis;
    ...
    *  @param hra hra, kterou budeme hrat
       ...
       // pokud neni vec v prostoru deojde ani k overeni jeji pouzitelnosti a je vypsana chybova hlaska 
        if( vec == null){
            ...
            switch(nazevVeci){
               ...
               /**          
               case "kamen":
                    vypis = helpKamen(aktualniProstor);//Vyuziva pomocnou metodu
                    break; */
    ...
    public String helpAvia(Prostor prostor){
       ...
       if(herniPlan.getUmisLitat()){
           ...
           airplane = new AudioClip(this.getClass().getResource("/zdroje/airplane.wav").toString());
           airplane.play();

           vrac = "\nNasedas do letadla a letis, pokusis se doletet co nejdal\n\n"
                  + "letadlo bylo pouzito, na dalsi let vsak neni benzin\n\n"
                   + "Pristals\n\n"
                   + cil.dlouhyPopis();
           return vrac;
    ...
    public String helpManual(Prostor prostor){
            manual = new AudioClip(this.getClass().getResource("/zdroje/manual.wav").toString());
           manual.play();
      String vrac;
    ...
    public String helpBud(Prostor prostor){
        ...
        Vec foto = new Vec(fotoHP,"Krasna fotografie Hercuila Poirota",true, "fotoHP.jpg");
     ...
     */  
     public String helpRukavice(Prostor prostor){
        ...
        String pom = prostor.getNazev();
        vrac = "\nZahral ste si ruce";
        if(batoh.isIn(fotoHP)==false &&pom.equals("vlak_do_Istanbulu")){
            vrac = "\nVelice dekuji, na oplatku vam doporucim aby jste ochutnal takzvany Kebab je to Turecka specialita";
        }
        else if(batoh.isIn(fotoHP) == true &&pom.equals("vlak_do_Istanbulu")){
            batoh.remVec(fotoHP);
            Vec foto2 = new Vec("PodepsanaFotografie","Krasna fotografie Hercuila Poirota s jeho podpisem",true, "PodepsanaFotografie.jpg");
